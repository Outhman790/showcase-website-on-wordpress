    ...
    if (  isset($_REQUEST['submit']) && isset( $_REQUEST['seller_auto_upload_nonce'] ) && $cur_user_id && ( !empty($front_auth) || wp_verify_nonce( $_REQUEST['seller_auto_upload_nonce'], 'seller_auto_upload' ))){
        ...
        if ($post_id) {
            ...
            $options = array(
                ...
                '_auto_color_int'      => sanitize_text_field( $_REQUEST['auto-color-int'] ),
                ...
                'pixad_auto_gallery_video' => (isset($_REQUEST['pixad_auto_gallery_video']) ? sanitize_text_field( $_REQUEST['pixad_auto_gallery_video'] ) : ''),
                'pixad_auto_gallery' => sanitize_text_field( $encode_gallery ),
            );
            for ($i=1; $i <= 80; $i++) {
                $options['_custom_'.$i] = (isset($_REQUEST['custom_'.$i]) ?  sanitize_text_field( $_REQUEST['custom_'.$i] ) : '')  ;
            }
            ...
            if ( is_wp_error( $_REQUEST['_thumbnail_id'] ) ) {
                esc_html_e('Error loading media file.', 'autozone');
            } else {
                set_post_thumbnail( $post_id, $_REQUEST['_thumbnail_id'] );
            }
            $save_data = $_FILES;
            ...
            if(function_exists('autozone_email_add_auto')){
...
function autozone_template_loader( $template ) {
    if(class_exists('PIXAD_Settings')){
        ...
        if($auto_listing_page){
            $auto_listing_page = autozone_wpml_get_all_post_id($auto_listing_page);
        }
...
function autozone_wpml_get_all_post_id($id, $post_type = 'page'){
    $page_translates_id = [];
            ...
            foreach( $ids as $id ){
                ...
                if(!empty($cur_id)){
                    ...
                    if($post->post_status === 'pending'){
                        $new_ids[] = $cur_id;
                        $html .= '<li class="notice-admin-pending templines_pending_notice">'.esc_html(get_the_author_meta('user_nicename', $post->post_author)).' added new car: <a href="'.get_permalink($cur_id).'">'.$post->post_title.'</li></a>';
                    }
                }
            }
        }
