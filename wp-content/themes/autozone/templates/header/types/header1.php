                ...
                <div class="header-topbarbox-2">
                    ...
                    <?php if($user->roles[0] == 'administrator' || $user->roles[0] == 'autodealer') {?>
                        <?php if(class_exists('BuddyPress') && class_exists('Youzify')){
                            ...
                            ?>
                            <a href="<?php echo esc_url($permalink)?>" class="fl-login-register--header"><i class="fa fa-user" aria-hidden="true"></i><?php echo esc_html($user->user_login, 'autozone');?></a>
                        <?php } else { ?>
            ...
            <div class="container ">
                <div class="header-navibox-1">
                    ...
                    <a class="navbar-brand scroll" href="<?php echo esc_url(home_url('/')) ?>" <?php echo wp_kses_post($autozone_logo_stl)?>>
                        ...
                        <img class="normal-logo" src="<?php echo esc_url($autozone_header['logo']) ?>" alt="logo" />
                        <?php else: ?>
                        <img class="normal-logo" src="<?php echo get_template_directory_uri(); ?>/images/logo-w.png" alt="logo" />
                        <?php endif ?>
                        <?php if ($autozone_header['logo_inverse']): ?>
                        <img class="scroll-logo hidden-xs" src="<?php echo esc_url($autozone_header['logo_inverse']) ?>" alt="logo" />
                        <?php else: ?>
                        <img class="scroll-logo hidden-xs" src="<?php echo get_template_directory_uri(); ?>/images/logo.png" alt="logo" />
                        <?php endif ?>
                    </a>
                </div>
                ...
                <div class="header-navibox-3">
                    ...
                    <ul class="nav navbar-nav hidden-xs">
                        ...
                        <?php if ( isset($autozone_opts['autos_sell_car_page']) && $autozone_opts['autos_sell_car_page'] != '' &&  $autozone_header['header_sell_button']) : ?>
                            ...
                            <?php if(class_exists('Youzify') && class_exists('Templines_Helper_Core_Addons')){?>
                                <?php if(is_user_logged_in()){?>
                                    <?php $permalink = bp_loggedin_user_domain() . '/add_autos/'; ?>
                                    <li class="no-hover header-button">
                                        <a class="autozone-sell-car" href="<?php echo esc_url($permalink) ?>"><?php echo wp_kses_post(autozone_get_option('header_button_text', esc_html__('Sell Your Car', 'autozone'))) ?></a>
                                    </li>
                                <?php } else { ?>
                                    <?php
                                    $youzify_login_page = get_option('youzify_membership_pages', true);
                                    $permalink = get_permalink($youzify_login_page['login']); ?>
                                    <li class="no-hover header-button">
                                        <a class="autozone-sell-car" href="<?php echo esc_url($permalink) ?>"><?php echo wp_kses_post(autozone_get_option('header_button_text', esc_html__('Sell Your Car', 'autozone'))) ?></a>
                                    </li>
                                <?php } ?>
